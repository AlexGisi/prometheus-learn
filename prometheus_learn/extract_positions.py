...
fp_out = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), FP_OUT))
...
for s in game_strs:
    ...
    all_positions.extend([{"result": res, "evaluation": float(m[0]), "fen": m[1]} for m in fen_matches])

print(f"Compiled {len(all_positions)} positions total.")
print("Filtering...")

balanced = list(filter(lambda d: abs(d['evaluation']) <= MAX_MATERIAL_DIFF, all_positions))
